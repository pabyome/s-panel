<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">MySQL Manager</h1>
        <p class="mt-1 text-sm text-gray-500">Manage your MySQL databases, users, and configuration.</p>
      </div>
    ...
    <div v-else-if="!status.installed" class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-900/5 p-12 text-center">
      ...
      <h3 class="mt-4 text-lg font-semibold text-gray-900">MySQL is not installed</h3>
      <p class="mt-2 text-sm text-gray-500">Get started by installing the MySQL service on your server.</p>
      <button
      ...
      >
        ...
        {{ installing ? 'Installing...' : 'Install MySQL' }}
      </button>
    </div>
    ...
    <template v-else>
      ...
      <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-900/5 overflow-hidden">
        ...
        <div v-if="currentTab === 'users'" class="p-6">
          <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-semibold text-gray-900">Users & Grants</h2>
          ...
          <div class="overflow-hidden rounded-xl ring-1 ring-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  <th class="py-3.5 pl-6 pr-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">User (Host)</th>
                ...
                <tr v-for="user in users" :key="user.name" class="hover:bg-gray-50 transition-colors">
                  ...
                  <td class="relative whitespace-nowrap py-4 pl-3 pr-6 text-right text-sm">
                    <div class="flex items-center justify-end gap-2">
                      ...
                      <button v-if="!['mysql.sys', 'root', 'debian-sys-maint'].includes(user.name.split('@')[0])" @click="confirmDeleteUser(user.name)" :disabled="deletingUserName !== null" class="rounded-lg p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-50" title="Delete">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                      </button>
                    </div>
                  </td>
          ...
          <div class="rounded-xl bg-gray-50 p-6">
            <div class="flex items-start justify-between">
              <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Allow connections from external hosts. Enabling this opens port 3306 and binds to 0.0.0.0.</p>
              </div>
              <button
          ...
          <div class="rounded-xl bg-gray-50 p-6">
            ...
            <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                ...
                <dd class="mt-1 font-mono text-sm text-gray-900">3306</dd>
              </div>
              <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200 sm:col-span-2">
                ...
                <dd class="mt-1 font-mono text-sm text-gray-900">{{ status.data_dir || '/var/lib/mysql' }}</dd>
              </div>
            </dl>
          </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-4">
              ...
              <div>
                ...
                <select v-model="connStringUser" class="block w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                  <option v-for="user in users" :key="user.name" :value="user.name.split('@')[0]">{{ user.name.split('@')[0] }}</option>
                </select>
              </div>
              <div>
        ...
        <div v-if="currentTab === 'advanced'" class="p-6 space-y-6">
              ...
              <button @click="openBackupAllModal" class="flex items-center justify-between rounded-lg bg-white p-4 ring-1 ring-gray-200 hover:bg-gray-50 transition-colors">
                <div class="flex items-center gap-3">
                  ...
                  <div class="text-left">
                    ...
                    <p class="text-xs text-gray-500">Create a full mysqldump backup</p>
                  </div>
                </div>
          ...
          <div class="rounded-xl bg-gray-50 p-6">
            <div class="flex items-start justify-between">
              <div>
                <h3 class="text-sm font-semibold text-gray-900">MySQL Version</h3>
                <p class="mt-1 text-sm text-gray-500">Current: v{{ status.version }}. Check for available updates.</p>
              </div>
              <button @click="checkMysqlUpdate" :disabled="checkingUpdate" class="inline-flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm font-medium text-gray-700 ring-1 ring-gray-200 hover:bg-gray-50 disabled:opacity-50">
            ...
            <div v-if="updateAvailable" class="mt-4 rounded-lg bg-amber-50 p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-amber-700 mt-0.5">Run <code class="bg-amber-100 px-1 rounded">sudo apt update && sudo apt upgrade mysql-server</code> to update.</p>
                </div>
              </div>
            </div>
          </div>
    ...
    <!-- Create User Modal -->
    <BaseModal :isOpen="isCreateUserOpen" @close="isCreateUserOpen = false" title="Create MySQL User" :showFooter="false">
      <form @submit.prevent="createUser" class="space-y-5">
        <div>
          ...
          <input
            ...
            placeholder="db_user"
          >
          <p class="mt-1 text-xs text-gray-500">Only letters, numbers, and underscores allowed. User will be created with '%' host.</p>
        </div>
        <div>
        ...
        <div class="rounded-xl bg-gray-50 p-4 space-y-3">
          ...
          <label class="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" v-model="createUserForm.grant_all" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600">
            <div>
              <span class="text-sm font-medium text-gray-900">Grant All Privileges</span>
              <p class="text-xs text-gray-500">Grants ALL PRIVILEGES ON *.* to this user</p>
            </div>
          </label>
        </div>
        <div class="flex gap-3 pt-2">
    ...
    <ConfirmModal
      ...
      message="Warning: Enabling remote access will expose your database to the internet. Password authentication will be enforced and port 3306 will be opened. Are you sure?"
      confirmText="Enable"
...
const createUserForm = ref({ name: '', password: '', grant_all: false })
...
const connectionString = computed(() => {
  ...
  const user = connStringUser.value || 'root'
...
const fetchStatus = async () => {
  try {
    const { data } = await axios.get('/api/v1/mysql/status')
    status.value = data
...
const fetchDatabases = async () => {
  try {
    const { data } = await axios.get('/api/v1/mysql/databases')
...
const fetchUsers = async () => {
  try {
    const { data } = await axios.get('/api/v1/mysql/users')
    users.value = data
  } catch (e) {
...
const installService = async () => {
  ...
  try {
    await axios.post('/api/v1/mysql/install')
    await fetchStatus()
    toast.success('MySQL installed successfully')
  } catch (e) {
...
const controlService = async (action) => {
  ...
  try {
    await axios.post(`/api/v1/mysql/control/${action}`)
...
const createDatabase = async () => {
  ...
  try {
    await axios.post('/api/v1/mysql/databases', createDbForm.value)
...
const deleteDatabase = async () => {
  ...
  try {
    await axios.delete(`/api/v1/mysql/databases/${dbToDelete.value}`)
...
const openCreateUserModal = () => {
  createUserForm.value = { name: '', password: '', grant_all: false }
...
const createUser = async () => {
  ...
  try {
    await axios.post('/api/v1/mysql/users', createUserForm.value)
...
const deleteUser = async () => {
  ...
  try {
    await axios.delete(`/api/v1/mysql/users/${userToDelete.value}`)
...
const changePassword = async () => {
  ...
  try {
    await axios.put(`/api/v1/mysql/users/${passForm.value.name}/password`, { password: passForm.value.password })
...
const grantAccess = async () => {
  ...
  try {
    await axios.post('/api/v1/mysql/grant', grantForm.value)
...
const toggleRemoteAccess = async () => {
  ...
  try {
    await axios.post('/api/v1/mysql/remote-access', { enable: newState })
    await fetchStatus()
...
const createBackup = async () => {
  ...
  try {
    const endpoint = selectedDb.value.name === 'all'
      ? '/api/v1/mysql/backup-all'
      : `/api/v1/mysql/databases/${selectedDb.value.name}/backup`
    await axios.post(endpoint, { format: 'sql' })
...
const checkMysqlUpdate = async () => {
  checkingUpdate.value = true
  try {
    const { data } = await axios.get('/api/v1/mysql/check-update')
    updateAvailable.value = data.available_version
    if (!data.available_version) {
      toast.success('MySQL is up to date')
    }
  } catch (e) {
...
// Global selectedDb ref for modals
